...
export default {
  ...
  async created() {
    ...
    console.log("j",this.user.displayName)
    this.partnerDocId = await this.getPartnerDoc()
    this.getPartnerContents();

    //  const pairRoomId = await 
     this.getPairRoomDocId(this.targetPartnerId);
    //  console.log(pairRoomId,"ありゃ")
     console.log(this.pairRoomId,"ありゃ")
    //  this.pairRoomId = pairRoomId
    //  this.submitDayRecord(pairRoomId);
...
if (storedDate === currentDate) {
  ...
  // 1:1の作業用ルームに移行する。、？→相手の参加まち？？時間合わせて・？？
  // それかペアルームからのボタンでページ移動。。？？うーん。。
  // 自然なのは、その場でタイマー稼働ではあるが、、まあきびいか・・？
...
localStorage.setItem('time', now);
    ...
    //フレンド部屋のdocidを取得する
    console.log("はて",this.pairRoomId)
      ...
      await firebase.firestore().collectionGroup('friend').where("friendId", "==", targetPartnerId).get()
        .then((querySnapshot) => {
          querySnapshot.forEach((doc) => {
            const data = doc.data();
            console.log("caaaaakk",data.pairRoomId)
...
// this.PartnerDocIDに各テキストフィールドから入力した内容を送信する


// ドキュメント内を監視して中身のfieldを取得する
getPartnerContents(){
